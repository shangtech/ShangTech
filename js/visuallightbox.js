...
(function(a) {
	a.fn.visualLightbox = function(ah) {
		...
		var m = ah.classNames;
		aO();
		var aK = au.length && au.get(0) != document ? au.get(0) : document
		...
		if (!document.getElementById(J("overlay"))) {
			...
			(ah.shadowLocation ? document.getElementById(J(ah.shadowLocation))
					: ao).appendChild(aS)
		}
		...
		var q = this.start = function(c) {
			...
			ab = null;
			aZ = 0;
			a("." + (c.attr("className") || c.get(0).className), au).each(
					function() {
						if (aT(this)) {
							e.push({
								link : aT(this),
		...
		function aD(t, be) {
			...
			if (ah.featBrowser) {
				...
				var a5 = (a3 + ah.borderSize * 2 + ah.breathingSize * 2)
						/ (bc + ah.borderSize * 2 + ah.breathingSize * 2 + p);
				if (a5 > a0) {
					var bb = bf.winWidth - ah.borderSize * 2 - ah.breathingSize
							* 2;
					var a6 = Math.round(bb / a7)
				} else {
					var a6 = bf.winHeight - ah.borderSize * 2
							- ah.breathingSize * 2 - p;
			...
			var a9 = i();
			if (ah.openByHover) {
				c = Math.min(Math.max(a(av).offset().left + a(av).width() / 2
						- a4 / 2, a9.x), a9.x + bf.winWidth - a4);
				a1 = Math.min(Math.max(a(av).offset().top + a(av).height() / 2
						- a2 / 2, a9.y), a9.y + bf.winHeight - a2)
			} else {
			...
			if (a2 > bf.winHeight) {
				a1 = Math.max(Math.min(aX("lightbox").offset().top, a9.y), a9.y
						+ bf.winHeight - a2)
			}
			var bd = ah.openByHover ? a4 + "px" : (/mobile.+safari/i
					.test(navigator.userAgent) ? bf.winWidth + "px" : "100%");
			...
			if (t == true) {
				...
				if (!be) {
					if (ah.startZoom) {
						...
						if (d) {
							d.css({
								width : (1 > bg ? "auto" : (d.width() / d
										.parent().width())
										* 100 + "%"),
								height : (1 > bg ? (d.height() / d.parent()
										.height())
										* 100 + "%" : "auto"),
								left : 0,
								top : 0
		...
		function x() {
			...
			if (e.length > 1) {
				var c = ah.strings.numDisplayPrefix + " " + parseInt(Y + 1)
						+ " " + ah.strings.numDisplaySeparator + " " + e.length;
		...
		function A(c) {
			if (c.pageX - aX("imageContainer").offset().left < aX(
